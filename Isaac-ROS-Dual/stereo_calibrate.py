...
class Config:
    ...
    square_mm: float = 28.0            # 格子边长 (mm)
    marker_mm: float = 18.0            # ChArUco marker 边长 (mm)，棋盘格模式下无用
    # 采集
    min_pairs: int = 25              # 最少采集对数
...
class CaptureSession:
    ...
    def _loop(self):
        ...
        while self._running:
            ...
            if self.cfg.rotate0:
                frame0 = _rotate(frame0, self.cfg.rotate0)
            if self.cfg.rotate1:
                frame1 = _rotate(frame1, self.cfg.rotate1)
            if self.cfg.swap_lr:
